  ...
  async update(
    ...
    const rows = await tx.$queryRaw<Branch[]>`
      UPDATE branches SET
        ...
        code             = COALESCE(${dto.code !== undefined ? dto.code : undefined}, code),
        is_headquarters  = COALESCE(${dto.isHeadquarters ?? null}::boolean, is_headquarters),
        country          = COALESCE(${dto.country !== undefined ? dto.country : undefined}, country),
        city             = COALESCE(${dto.city !== undefined ? dto.city : undefined}, city),
        state            = COALESCE(${dto.state !== undefined ? dto.state : undefined}, state),
        address          = COALESCE(${dto.address !== undefined ? dto.address : undefined}, address),
        postal_code      = COALESCE(${dto.postalCode !== undefined ? dto.postalCode : undefined}, postal_code),
        timezone         = COALESCE(${dto.timezone ?? null}, timezone),
        phone            = COALESCE(${dto.phone !== undefined ? dto.phone : undefined}, phone),
        email            = COALESCE(${dto.email !== undefined ? dto.email : undefined}, email),

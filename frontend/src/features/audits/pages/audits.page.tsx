...
export function AuditsPage(): JSX.Element {
  ...
  function handleDelete(id: string) {
    deleteAudit(id, { onSuccess: () => setConfirmDeleteId(null) });
  }
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Plan and track audit engagements and their findings.
            </p>
          </div>
          <button
            onClick={openCreate}
        ...
        <div className="mb-4 flex flex-wrap items-center gap-3">
          ...
          <select
            value={statusFilter}
            onChange={(e) => resetToFirstPage(setStatusFilter)(e.target.value as AuditStatus | '')}
          ...
          <select
            value={typeFilter}
            onChange={(e) => resetToFirstPage(setTypeFilter)(e.target.value as AuditType | '')}
            className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-600"
          >
            ...
            <table className="w-full text-sm">
              <thead className="border-b border-slate-200 bg-slate-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left font-medium text-slate-500">Auditor</th>
                  <th className="px-6 py-3 text-left font-medium text-slate-500">Dates</th>
                  <th className="px-4 py-3 text-right font-medium text-slate-500">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {audits.map((audit) => {
                  const typeCfg = AUDIT_TYPE_CONFIG[audit.auditType];
                  ...
                  return (
                    <tr key={audit.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4">
                        ...
                        {audit.ownerName && (
                          <div className="text-xs text-slate-400">{audit.ownerName}</div>
                        )}
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-slate-600">
                        {audit.frameworkRef ?? <span className="text-slate-400">—</span>}
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-slate-600">
                        {audit.auditorName ?? <span className="text-slate-400">—</span>}
                      </td>
                      <td className="px-6 py-4 text-slate-600">
                      ...
                      <td className="px-4 py-4 text-right">
                        <div className="relative inline-block">
                          <button
                            onClick={() => setMenuOpenId(menuOpenId === audit.id ? null : audit.id)}
                          ...
                          {menuOpenId === audit.id && (
                            <div className="absolute right-0 top-full z-10 mt-1 w-44 rounded-lg border border-slate-200 bg-white shadow-lg py-1">
                              <button
                                onClick={() => { openEdit(audit); setMenuOpenId(null); }}
                                className="flex w-full items-center gap-2 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50"
                              >
                                <Pencil className="h-4 w-4" /> Edit
                              </button>
                              <button
                                onClick={() => { setConfirmDeleteId(audit.id); setMenuOpenId(null); }}
        ...
        {!isLoading && !isError && audits.length > 0 && (
          <div className="mt-3 flex items-center justify-between">
            <p className="text-xs text-slate-400">
              Showing {(page - 1) * PAGE_SIZE + 1}–{(page - 1) * PAGE_SIZE + audits.length} of {total} audits
            </p>
      ...
      {confirmDeleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
            <h3 className="text-base font-semibold text-slate-900">Delete Audit?</h3>
            <p className="mt-2 text-sm text-slate-600">
              This will permanently delete the audit and its findings. This action cannot be undone.
            </p>
...
function ErrorState({ onRetry }: { onRetry: () => void }): JSX.Element {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <h3 className="text-base font-semibold text-slate-900">Couldn't load audits</h3>
      <p className="mt-1 text-sm text-slate-500">
        Something went wrong while fetching audits.
      </p>
      <button
        onClick={onRetry}
...
function EmptyState({ onAdd }: { onAdd: () => void }): JSX.Element {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="mt-1 text-sm text-slate-500">
        Create your first audit engagement to start tracking findings.
      </p>
      <button
        onClick={onAdd}
